...
-spec bulk_docs(#erls_params{}, list(), binary()) -> {ok, list()} | {error, any()}.
bulk_docs(Params, IndexTypeIdJsonTuples, Operation) ->
    Body = lists:map(fun
			 Build({Index, Type, Id, Doc}) ->
			     Build({Index, Type, Id, [], Doc});
			 Build({Index, Type, Id, HeaderInformation, Doc}) ->
			     Header = bulk_docs_header(Operation,Index, Type, Id, HeaderInformation),
			     [ Header, <<"\n">>, maybe_encode_doc(Doc), <<"\n">> ]
		     end, IndexTypeIdJsonTuples),
    erls_resource:post(Params, <<"/_bulk">>, [], [], iolist_to_binary(Body), Params#erls_params.http_client_options).
...
bulk_index_docs_header(Index, Type, Id, HeaderInformation) ->
    bulk_docs_header(<<"index">>, Index, Type, Id, HeaderInformation).


-spec bulk_docs_header(binary(),binary(), binary(), binary(), list()) -> binary().
bulk_docs_header(Operation, Index, Type, Id, HeaderInformation) ->
    IndexHeaderJson1 = [
        {<<"_index">>, Index}
        ,{<<"_type">>, Type}
        | HeaderInformation
    ...
    %% we cannot use erls_json to generate this, see the doc string for `erls_json:encode/1'                                                                                                                                                                                  
    jsx:encode([{Operation, IndexHeaderJson2}]).
